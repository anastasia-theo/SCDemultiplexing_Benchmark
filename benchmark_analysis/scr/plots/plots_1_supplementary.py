...
elapsed_times_tool1 = [
    [8, 9, 9, 25, 381],  # Dataset 1 subs
    [7, 7, 8, 22, 335],  # Dataset 2 subs+indels
    [274, 271, 268, 472, 2097],  # Dataset 3 signal_seq
    [505, 545, 595, 879, 2453]  # Dataset 4 nki
]
...
# Plot elapsed time for the first dataset (top-left)
axes[0, 0].plot(mismatches, elapsed_times_tool1[0], label=f"{datasets[0]} - splitcode", 
                color=tool_colors[0], linestyle=dataset_line_styles[0], linewidth=2)
axes[0, 0].plot(mismatches, elapsed_times_tool2[0], label=f"{datasets[0]} - SCDemultiplexing Pipeline", 
                color=tool_colors[1], linestyle=dataset_line_styles[0], linewidth=2)

axes[0, 0].set_xlabel("Number of Mismatches", fontsize=12)
axes[0, 0].set_ylabel("Elapsed Time (s)", fontsize=12)
axes[0, 0].set_title("Elapsed Time: Artificial Subs", fontsize=14)
...
# Plot elapsed time for the 3rd dataset (top-right)
axes[0, 1].plot(mismatches, elapsed_times_tool1[2], label=f"{datasets[2]} - splitcode", 
                color=tool_colors[0], linestyle=dataset_line_styles[2], linewidth=2)
axes[0, 1].plot(mismatches, elapsed_times_tool2[2], label=f"{datasets[2]} - SCDemultiplexing Pipeline", 
                color=tool_colors[1], linestyle=dataset_line_styles[2], linewidth=2)

axes[0, 1].set_xlabel("Number of Mismatches", fontsize=12)
axes[0, 1].set_ylabel("Elapsed Time (m)", fontsize=12)  # Updated y-axis label
axes[0, 1].set_title("Elapsed Time: SIGNAL_seq", fontsize=14)
...
# Plot max memory for the first dataset (middle-left)
axes[1, 0].plot(mismatches, max_memory_tool1[0], label=f"{datasets[0]} - splitcode", 
                color=tool_colors[0], linestyle=dataset_line_styles[0], linewidth=2)
axes[1, 0].plot(mismatches, max_memory_tool2[0], label=f"{datasets[0]} - SCDemultiplexing Pipeline", 
                color=tool_colors[1], linestyle=dataset_line_styles[0], linewidth=2)

axes[1, 0].set_xlabel("Number of Mismatches", fontsize=12)
axes[1, 0].set_ylabel("Max Memory (GB)", fontsize=12)
axes[1, 0].set_title("Max Memory: Artificial Subs", fontsize=14)
...
# Plot max memory for the third dataset (middle-right)
axes[1, 1].plot(mismatches, max_memory_tool1[2], label=f"{datasets[2]} - splitcode", 
                color=tool_colors[0], linestyle=dataset_line_styles[2], linewidth=2)
axes[1, 1].plot(mismatches, max_memory_tool2[2], label=f"{datasets[2]} - SCDemultiplexing Pipeline", 
                color=tool_colors[1], linestyle=dataset_line_styles[2], linewidth=2)

axes[1, 1].set_xlabel("Number of Mismatches", fontsize=12)
axes[1, 1].set_ylabel("Max Memory (GB)", fontsize=12)
axes[1, 1].set_title("Max Memory: SIGNAL_seq", fontsize=14)
...
# Save the figure
output_path = "/scistor/informatica/ath100/SCDemultiplexing_Benchmark/benchmark_analysis/results/plots/combined_comparison_subplots_main_new.png"
