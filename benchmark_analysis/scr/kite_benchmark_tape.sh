#!/bin/bash
#SBATCH --job-name=kite_benchmark_tape
#SBATCH --error=/scistor/informatica/ath100/SCDemultiplexing_Benchmark/benchmark_analysis/results/kite/tape_dataset/kite_benchmark_tape_%A%a.err  # Error file location
#SBATCH --output=/scistor/informatica/ath100/SCDemultiplexing_Benchmark/benchmark_analysis/results/kite/tape_dataset/kite_benchmark_tape_%A%a.out # Output file location
...
FASTQ_DIR="/scistor/informatica/ath100/SCDemultiplexing_Benchmark/benchmark_analysis/raw_data/tape_data/"
RESULTS_DIR="/scistor/informatica/ath100/SCDemultiplexing_Benchmark/benchmark_analysis/results/kite/tape_dataset"
FEATURES_SCRIPT="/scistor/informatica/ath100/SCDemultiplexing_Benchmark/benchmark_analysis/scr/kITE_tape_features_creation.py"
...
/usr/bin/time -v bash -c "
  ...
  kb count --h5ad \
           ...
           -o $RESULTS_DIR/ \
           -w $RESULTS_DIR/whitelist_reverse.txt \
           -g $RESULTS_DIR/t2g.txt \
           -x 1,10,18,1,48,56,1,78,86:1,0,10:0,0,0 \
           --workflow kite -t 2 --keep-tmp --overwrite \
           $FASTQ_DIR/filtered_SRR28056728_1.fastq \
           $FASTQ_DIR/filtered_SRR28056728_2.fastq | tee -a $OUTPUT_LOG
