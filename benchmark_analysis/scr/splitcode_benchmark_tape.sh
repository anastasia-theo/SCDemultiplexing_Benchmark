#!/bin/bash
#SBATCH --job-name=splitcode_benchmark_tape
#SBATCH --error=/scistor/informatica/ath100/SCDemultiplexing_Benchmark/benchmark_analysis/results/splitcode_results/tape_dataset/splitcode_benchmark_tape_%A%a.err  # Error file location
#SBATCH --output=/scistor/informatica/ath100/SCDemultiplexing_Benchmark/benchmark_analysis/results/splitcode_results/tape_dataset/splitcode_benchmark_tape_%A%a.out # Output file location
...
CONFIG_FILE="/scistor/informatica/ath100/SCDemultiplexing_Benchmark/benchmark_analysis/raw_data/splitcode/config_tape.txt"
TEMP_CONFIG="/scistor/informatica/ath100/SCDemultiplexing_Benchmark/benchmark_analysis/raw_data/splitcode/temp_config_tape.txt"
FASTQ_FILE="/scistor/informatica/ath100/SCDemultiplexing_Benchmark/benchmark_analysis/raw_data/tape_data/SRR28056728_merged.fastq"
RESULTS_DIR="/scistor/informatica/ath100/SCDemultiplexing_Benchmark/benchmark_analysis/results/splitcode_results/tape_dataset"
...
update_mismatches_in_config() {
    ...
    awk -v mismatch="$mismatch" '
        BEGIN {OFS=FS="\t"} 
        # If the line starts with "grp_A", keep it unchanged
        /^grp_A/ {print; next}
...
# If the tool runs successfully, extract time/memory usage
    ...
    BARCODES_FILE="/scistor/informatica/ath100/SCDemultiplexing_Benchmark/benchmark_analysis/raw_data/tape_data/barcodes_tape.txt"
    MAPPING_FILTERED="$RESULTS_DIR/mapping_filtered_${mismatch}.txt"
    FILTERING_SCRIPT="/scistor/informatica/ath100/SCDemultiplexing_Benchmark/benchmark_analysis/scr/splitcode_barcode_filtering_tape.py"
    ...
    if [ ! -s "$MAPPING_FILTERED" ]; then
        echo "No sequences with all 4 variable barcodes found for mismatch $mismatch. Skipping to the next mismatch."
        echo "$mismatch 0" >> "$FILTERED_PERCENT_LOG"
        continue
    fi
...
# Path to output plot
# OUTPUT_PLOT_PATH="$RESULTS_DIR/plots/metrics_splitcode.png"

# Call the Python plotting script
# python3 /home/mrhin/SCDemultiplexing_Benchmark/benchmark_analysis/scr/plot_creation.py "$TIME_LOG" "$MEMORY_LOG" "$ACCURACY_LOG" "$FILTERED_PERCENT_LOG" "$OUTPUT_PLOT_PATH"

# plot notification
# echo "Plots have been generated and saved."
