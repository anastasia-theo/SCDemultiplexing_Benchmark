...
#             if len(columns) >= 7:
#                 combined_seq = columns[0] + columns[1] + columns[2] + columns[3] + columns[4] + columns[5]
...
def compare_reads(scd_counts, split_counts, read_counts_output, reads_shared, mismatch):
    ...
    # Count shared reads (those that appear in both SCD and Split)
    shared_reads = 0
    shared_details = []  # List to keep track of shared reads and their counts in both tools
    for seq in scd_counts:
        if seq in split_counts:
            count_scd = scd_counts[seq]
            count_split = split_counts[seq]
            shared_count = min(count_scd, count_split)  # Shared count is the minimum of the two
            shared_reads += shared_count
            shared_details.append((seq, count_scd, count_split, shared_count))
    ...
    # Write details of shared reads and their counts in both tools
    with open(reads_shared, 'w') as out:  # Create a new file for each mismatch
        for seq, count_scd, count_split, shared_count in shared_details:
            out.write(f"{seq}\t{count_scd}\t{count_split}\t{shared_count}\n")
...
split_output = sys.argv[4]
read_counts_output = sys.argv[5]
mismatch = sys.argv[7]  # Mismatch value

# Generate reads_shared filename based on mismatch value
reads_shared = f"reads_shared_{mismatch}.txt"
